...
public class View {
    ...
    private static String getChar(int x, int y){
        ...
        for (MatherUnit human: Main.allHero) {
            if (human.getPos()[0] == x && human.getPos()[1] == y){
                ...
                if (Main.blueHero.contains(human)) out = "|" + (AnsiColors.ANSI_GREEN + human.getInfo().charAt(0) + AnsiColors.ANSI_RESET);
                if (Main.redHero.contains(human)) out = "|" + (AnsiColors.ANSI_BLUE + human.getInfo().charAt(0) + AnsiColors.ANSI_RESET);
                break;
            }
        }
        return out;
    ...
    public static void view() {
        ...
        step++;
        Main.allHero.forEach((v) -> l[0] = Math.max(l[0], v.toString().length()));
